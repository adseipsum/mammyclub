<?php
  ...
  if(isset($_GET['css-in-js'])) {
    $cssdir = dirname(__FILE__) . '/web';
  }
  ...
  $contents = combine($type, $path, $files);
  ...
  function compress($data, $encoding) {
    if ($encoding != "none") {
      if ($encoding == "gzip") {
        ...
        $data = gzencode($data, 6, FORCE_DEFLATE);
      }
    }
    return $data;
  }
   ...
   * @return string
   */
  function combine($type, $basePath, $files) {
    $result = "";
    foreach($files as $file) {
      $path = realpath($basePath . '/' . trim($file));
      if (!$path) {
      ...
      if (!empty($path) && preg_match("/$type$/", $path)) {
