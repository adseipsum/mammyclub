...
if ( ! function_exists('ajax_result_redirect_current_url_top')) {
  function ajax_result_redirect_current_url_top() {
    return json_encode(array("redirect" => 'window.location.href', 'top' => TRUE));
  }
}


if (!function_exists('uni_redirect')) {
  function uni_redirect($url, $top = FALSE) {
    if(!preg_match("/^http(s?):\/\//", $url)) {
      $url = site_url($url);
    }
    if (is_ajax()) {
      if ($top){
        die('{"redirect":"' . $url . '", "top":"true"}');
      } else {
        die('{"redirect":"' . $url . '"}');
      }
    } else {
