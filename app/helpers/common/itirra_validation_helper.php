 ...
 * @param array $keys
 */
if (!function_exists('simple_validate_get')) {
  function simple_validate_get($keys = array()) {
...
if (!function_exists('save_post')) {
  function save_post($exclude = array(), $foralong = false) {
    ...
    $session = $CI->session;
    if ($foralong) {
      $session->set_userdata('saved_post', serialize(array_diff_key($_POST, assoc_array_from_values($exclude))));
    } else {
      $session->set_flashdata('saved_post', serialize(array_diff_key($_POST, assoc_array_from_values($exclude))));
    }
...
if (!function_exists('get_saved_post')) {
  function get_saved_post() {
    $CI =& get_instance();
    $session = $CI->session;
    $post = unserialize($session->flashdata('saved_post'));
    if (!$post) $post = unserialize($session->userdata('saved_post'));
    return $post;
  }
}
...
if (!function_exists('fill_form_with_saved_post')) {
  function fill_form_with_saved_post($formId, $processHiddenFields = TRUE, $appendScriptTag = TRUE) {
    $js = '';
    $post = get_saved_post();
    ...
    if (!empty($post)) {
      ...
      foreach ($post as $k => $v) {
        if (!($v instanceof Doctrine_Null)) {
          if (!is_array($v)) {
            if ($processHiddenFields) {
              $js .= 'el = $("#' . $formId . '").find(\'[name="' . $k . '"]:first\');' . "\n";
            } else {
              $js .= 'el = $("#' . $formId . '").find(\'[name="' . $k . '"]:visible:first\');' . "\n";
            }
            ...
            $js .= 'if(el && el[0] && el[0].nodeName.toLowerCase() == "select") {
              el.val("' . str_replace(array('<', '>'), '', addslashes($v)) . '");
            ...
            if ($processHiddenFields) {
              $js .= 'el = $("#' . $formId . '").find(\'[name="' . $k . '[]"]\');' . "\n";
            } else {
              $js .= 'el = $("#' . $formId . '").find(\'[name="' . $k . '[]"]:visible\');' . "\n";
            }
            foreach ($v as $i => $vv) {
              $js .= '$(el[' . $i . ']).val("' . addslashes($vv) . '");' . "\n";
            }
          }
        }
      }
