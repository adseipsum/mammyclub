...
if (!function_exists('lang')) {
  function lang($message, $params = array(), $maxChars = 200, $stripTags = TRUE) {
    $ci = get_instance();
    if (ENV == 'DEV') {
      if (isset($ci->language) && !isset($ci->language->usedkeys)) {
        $ci->language->usedkeys = array();
      }
      if (isset($ci->language) && !array_search($message, $ci->language->usedkeys)) {
        $ci->language->usedkeys[] = $message;
      }
    }
...
* @return array
*/
if (!function_exists('get_alphabet_en')) {
  function get_alphabet_en() {
...
* @return array
*/
if (!function_exists('get_alphabet_ru')) {
  function get_alphabet_ru() {
...
* @return array
*/
if (!function_exists('get_alphabet_ua')) {
  function get_alphabet_ua() {
    return array('а','б','в','г','ґ','д','е','є','ж','з','и','і','ї','й','к','л','м','н','о','п','р','с','т','у','ф','х','ц','ч','ш','щ','ь','ю','я');
  }
}
...
if (!function_exists('to_translit')) {
  function to_translit($string) {
    ...
    $replace = array('zh','yo','j','yu','ya','e','sh','sch','ch','Zh','Yo','J','Yu','Ya','E','Sch','Sh','Ch','I', 'i', 'G', 'g', 'Ye', 'ye', 'Yi', 'yi');
...
if (!function_exists('from_translit')) {
  if (!function_exists('from_translit')) {
    function from_translit($string) {
      $search = array('zh','yo','j','yu','ya','sh','sch','ch','Zh','Yo','J','Yu','Ya','Sch','Sh','Ch','I','i');
      $replace = array( 'ж','ё','й','ю','я','ш','щ','ч','Ж','Ё','Й','Ю','Я','Щ','Ш', 'Ч','І','і');
      $string = str_replace($search, $replace, $string);
...
if (!function_exists('lang_url')) {
  function lang_url($string, $startsWith = null, $russian = FALSE) {
    $russianUrls = FALSE;
    if ($russian || ($startsWith && preg_match('/[А-Яа-яЁё]/u', $startsWith))) {
      $russianUrls = TRUE;
    }
    ...
    if (!$russianUrls) {
      $string = preg_replace("/[^a-z0-9-\s]/", "", $string); //remove all non-alphanumeric characters except the hyphen
    } else {
      $string = preg_replace("/[^a-z0-9а-я-\s]/u", "", $string); //remove all non-alphanumeric characters except the hyphen
    }
    $string = str_replace("\n", "", $string);
...
if (!function_exists('slug')) {
  function slug($string, $russian = FALSE) {
    $string = trim(strip_tags($string));
    if (!$russian) {
      $string = str_replace('&', 'and', $string);
      $string = to_translit($string);
    ...
    if (!$russian) {
      ...
      $string = preg_replace("/[^а-я0-9-\s]/u", "", $string); //remove all non-alphanumeric characters except the hyphen
    }
    $string = str_replace("\n", "", $string);
    $string = str_replace("\r", "", $string);
    $string = str_replace(' ', '-', $string);
