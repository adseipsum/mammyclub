...
if (!function_exists('site_img')) {
  function site_img($imgFile, $imagesFolder = TRUE, $domain = NULL) {
...
if (!function_exists('site_image_thumb_url')) {
  function site_image_thumb_url($thumbName, $image, $imagesFolder = TRUE, $checkForExists = TRUE) {
    if(isset($image['domain'])) {
      if($checkForExists == FALSE) {
        return site_img($image["web_path"] . str_replace($image['extension'], $thumbName . $image['extension'], $image["file_name"]), $imagesFolder, $image['domain']);
      }
      return site_img($image["web_path"] . $image["file_name"], $imagesFolder, $image['domain']);
    } else {
