...
/**
 * highlight
 * Wraps found $word in a given $string with the highlight tag
 * @param str
 */
if (!function_exists('highlight')) {
  function highlight($str, $word, $fromBeginningOnly = FALSE, $tagOpen = '<span class="highlight">', $tagClose = '</span>') {
    if(empty($str)) {
      return '';
    }
    if(empty($word)){
    ...
    if (is_array($word)) {
      foreach ($word as $w) {
        $str = preg_replace('/' . ($fromBeginningOnly ? '^' : '') . '('.preg_quote($w, '/').')/iu', $tagOpen."\\1".$tagClose, $str); 
      } 
      return $str; 
    } else {
      return preg_replace('/' . ($fromBeginningOnly ? '^' : '') . '('.preg_quote($word, '/').')/iu', $tagOpen."\\1".$tagClose, $str);
    }
