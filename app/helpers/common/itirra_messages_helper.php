...
if (!function_exists('html_flash_message')) {
  function html_flash_message($message = null, $closeBtn = TRUE) {
    $html = "";
...
if (!function_exists('set_flash_validation_errors')) {
  function set_flash_validation_errors($vErrors) {
    ...
    foreach ($vErrors as $fKey => $errors) {
      if(lang_exists('field.' . $fKey)) {
        $fieldName = lang('field.' . $fKey);
      } else {
        $fieldName = ucwords(str_replace('_', ' ', $fKey));
      }
      $arg = array('field_name' => $fieldName);
      $errors = $errors[0];
 ...
 */
if (!function_exists('keep_flash_message')) {
  function keep_flash_message() {
    $CI =& get_instance();
    return $CI->session->keep_flashdata('message');
  }
}
