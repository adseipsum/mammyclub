...
if (!function_exists('generate_years')) {
  function generate_years($startYear = 1900, $endYear = NULL, $reverseOrder = false) {
    if (!$endYear) {
      $endYear = (int)date('Y');
    }
    $result = array();
...
if (!function_exists('generate_months')) {
  function generate_months($langCode = 'ru', $genitive = false) {
    //genitive = roditelniy padezh
    $result = array();
    switch ($langCode) {
      case 'ru':
        if ($genitive) {
...
if (!function_exists('age')) {
  function age($birthday) {
    ...
    if (is_php_version_later_than("5.3")) {
      // loading date_lang
      $CI =& get_instance();
      $CI->lang->load('date');
      ...
      $birthdayDate = DateTime::createFromFormat('Y-m-d', $birthday, $timeZone);
      if (!$birthdayDate) {
        return;
      }
      $age = $birthdayDate->diff(new DateTime('now', $timeZone));

      $ageYears = $age->format("%y");
      if ($ageYears) {
        return number_noun($ageYears, 'years');
      }

      $ageMonthes = $age->format("%m");
      if ($ageMonthes) {
        return number_noun($ageMonthes, 'month');
      }

      $ageDays = $age->format("%d");
      return number_noun($ageDays, 'day');
    }
...
/**
 * Makes an array from date, whitch format you need to specify (e.g. %Y-%m-%d)
 * E.g. date = 2012-07-25,
 * it returns: array("tm_mday" => 25,
"tm_mon"  => 07,
"tm_year" => 2012)
 */
