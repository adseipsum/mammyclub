...
>
  <head>
    ...
    <?/*
    <? if(url_equals('беременность-по-неделям') || url_equals('беременность-по-неделям1') || url_equals('беременность-по-неделям2')): ?>
      <link href="<?=site_url('беременность-по-неделям')?>" rel="canonical">
    <? endif;?>
    */?>
    ...
    <link async href='https://fonts.googleapis.com/css?family=PT+Sans:400,400italic,700,700italic&subset=latin,cyrillic-ext' rel='stylesheet' type='text/css'>

    <? $css = "common/zero.css, common/messages.css, common/ajax-popup2.css, html-content.css, buttons.css, ezmark.css, style.css"; ?>
    <link async type="text/css" rel="stylesheet" media="screen" href="<?=site_css($css);?>"/>
