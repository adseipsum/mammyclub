<div class="content default-box">
  ...
  <div class="inner export">
    <form action="<?=site_url($processLink);?>" method="post" class="form validate" autocomplete="off" enctype="multipart/form-data" id="import_form">
      ...
      <input type="hidden" name="import_type" value="edit_only" />
      ...
      <div class="fields">
        ...
        <div class="checkboxes">
          ...
          <? foreach ($fields as $k => $params): ?>
            <div class="ex-box">
              <input class="checkbox <?=($params['type'] == 'image')?'image':''?>" type="checkbox" name="<?=$k?>" id="<?=$entityName?>_<?=$k?>" value="1" checked="checked" <? if(in_array($k, $required['simple'])): ?>disabled="disabled"<? endif; ?> />
