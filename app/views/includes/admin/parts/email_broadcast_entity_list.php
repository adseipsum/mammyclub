  ...
  <?if (isset($entities) && sizeof($entities) > 0):?>
    ...
    <form action="<?=site_url($adminBaseRoute . '/email_broadcast_' . $entityName . '/exclude')?>" class="form" autocomplete="off" method="post">
     <table id="list_<?=$entityName;?>" class="table">
            ...
            <?foreach($params as $param):?>
              ...
              <? else: ?>
                  ...
                  <? else: ?>
                    ...
                    <? endif; ?>
                  	<td><? if(isset($search["search_string"]) && in_array($param, explode(',', $search['search_in']))): ?><?=highlight_phrase(htmlspecialchars($el[$param]), $search["search_string"],'<span class="red">', '</span>')?><?else:?><?=htmlspecialchars($el[$param])?><? endif; ?></td>
                  <? endif; ?>
                <? else : ?>
                	<td></td>
                <? endif; ?>
        ...
        <tfoot>
          <tr>
            <td colspan="<?=++$colSpan?>"><?=lang("admin.total")?>: <?=$totalCount;?></td>
          </tr>
        </tfoot>
      </table>
