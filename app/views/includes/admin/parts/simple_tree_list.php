...
<div class="inner">
  <div class="holder-row">
    <? $first = TRUE; ?>
    <? foreach($params as $p): ?>
      <? /*<span class="" style="float: left; width:<? if($first): ?>400px; margin-left: 10px;<? else: ?>100px; text-align: center;<? endif; ?>">*/?>
      <span class="holder-v <? if($first): ?>left<? else: ?>right<? endif; ?>">
        <? if(is_array($p)): ?>
          <? $tmp = ""; ?>
          <? foreach ($p as $pk => $pv): ?>
          <? $tmp .= $pk . '.' . $pv; ?>
  ...
  <? function nested_array_loop_2(&$html = null, $tree, $actions, $lang, $first = FALSE, $params, $maxLevel = null) { ?>
    ...
    <?$count = 0;
      foreach($tree as $item){
        $count++; ?>
        <? $item = array_make_plain_with_dots($item); ?>
        <li id="item_<?=$item['id'];?>" <? if($item['level'] == 0): ?>class="<?= ($count % 2 == 0) ? 'even' : 'odd' ?> <? endif; ?>">
          <div class="listCat">
            <table cellpadding="0" cellspacing="0" border="0" width="100%">
              <tr>
                ...
                <? foreach ($params as $lp): ?>
                  ...
                  <? elseif (isset($item[$lp]) && is_bool($item[$lp])): ?>
                    ...
                    <? if ($k == "add_child"): ?>
                      ...
                      <? else: ?>
                       <? if ($k == "delete"): ?>
                         ...
                         <? endif;?>
                        <? else: ?>
                          <? if ($k == 'edit' && (!isset($item['can_be_edited']) || (isset($item['can_be_edited']) && $item['can_be_edited'] == TRUE))): ?>
                            <a href="<?=site_url($v . "/" . $item['id']);?>" ><?=$lang->line("admin." . $k)?></a><?if ($countActions < count($actions)):?>&nbsp;|<?endif;?>
                          <? elseif ($k != 'edit'): ?>
        	                  <a href="<?=site_url($v . "/" . $item['id']);?>" ><?=$lang->line("admin." . $k)?></a><?if ($countActions < count($actions)):?>&nbsp;|<?endif;?>
        	                <? endif; ?>
  ...
  <? if (count($entities) > 0):?>
    <div class="holder-foot">
    	<a href="#" class="show-all"><?=lang("admin.show_all");?></a>&nbsp;&nbsp;&nbsp;<a href="#" class="hide-all"><?=lang("admin.hide_all");?></a>
    </div>
  <? endif; ?>
</div>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    if (item_id) {
      $('#item_' + item_id).parents('ul').show();
      $('#item_' + item_id).parents('li').children('.listCat').find('.plus-minus').html('-');
    }
