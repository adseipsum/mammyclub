...
<? else: ?>
  ...
  <div class="group">
    <label class="label" for="<?=str_replace('.', '_', $id)?>"><?=lang('admin.add_edit.broadcast.recipents_list')?></label>     
    <textarea id="<?=str_replace('.', '_', $id)?>"
              class="text-area <?=isset($params['class']) ? $params['class'] : ""?>"
              name="recipents_list"
              <?=$attrs?>></textarea>
    
    <?if ( ! empty($message)) :?><span class="description"><?=lang('admin.add_edit.broadcast.recipents_list.description')?></span><?endif?>              
  </div>   
