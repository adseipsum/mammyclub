...
/**
 * OrderConfirmedBroadcastManager
 * This class has been auto-generated by Itirra
 */
require_once APPPATH . 'logic/base/BaseManager.php';
class OrderConfirmedBroadcastManager extends BaseManager {
  ...
  public function sendBySiteOrder($siteOrderId) {
    ...
    $broadcast = ManagerHolder::get('OrderConfirmedBroadcast')->getAll('e.*');
    if (empty($broadcast)) {
      return false;
    }
    ...
    $broadcastData = array(
      ...
      'type' => ORDER_CONFIRMED_BROADCAST
    );

    $broadcastId = ManagerHolder::get('MandrillBroadcast')->insert($broadcastData);

    // Collect data to array
    $viewData = ManagerHolder::get('OrderConfirmedBroadcast')->createBroadcastContent($broadcast, $user, $siteOrder);
    ...
    ManagerHolder::get('EmailNotice')->sendNoticeAboutBroadcastEnd(ORDER_CONFIRMED_BROADCAST, 1);
...
//    }
  ...
  public function createBroadcastContent($broadcast, $user, $siteorder) {
    ...
    if (!empty($siteorder['shipment_date'])) {
      $siteorder['shipment_date'] = date('d.m.Y', strtotime($siteorder['shipment_date']));
    }

    // Proccess data
    foreach ($viewData as &$data) {
      if (!empty($data)) {
        if (strpos($data, '{liqpay_checkout_link}') !== FALSE) {
          $liqpayCheckoutLink = shop_url('liqpay-checkout/' . $siteorder['id']);
          $data = str_replace('{liqpay_checkout_link}', $liqpayCheckoutLink, $data);
        }
        if(strpos($data, '{user_comment}') !== FALSE) {
          $comment = '';
          if(!empty($siteorder['comment'])) {
            $comment = 'Ваш комментарий к заказу:<br/>';
            $comment .= $siteorder['comment'];
          }
          $data = str_replace('{user_comment}', $comment, $data);
        }
