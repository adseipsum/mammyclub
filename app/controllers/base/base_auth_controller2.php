 ...
 */
class Base_Auth_Controller2 extends Base_Controller {
   ...
   * Constructor.
   */
  public function Base_Auth_Controller2() {
  ...
  public function login_process() {
    ...
    // -- VALIDATION
      ...
      uni_redirect($this->authConfig['url_login'], FALSE, array('captcha' => $captcha));
  ...
  protected function postLoginProcess() {
    $this->authEntity = $this->auth->getAuthEntity();
    if ($this->authConfig['email_confirmation'] == TRUE && !$this->authEntity['email_confirmed']) {
      uni_redirect($this->authConfig['url_email_confirm']);
    }
    if ($this->authConfig['phone_confirmation'] == TRUE && !$this->authEntity['phone_confirmed']) {
      uni_redirect($this->authConfig['url_phone_confirm']);
    }
  ...
  public function forgot_password_process() {
  	if ($this->isLoggedIn) show_404();
  	try {
  		$this->auth->forgotPassword($_POST);
  		set_flash_notice('auth.message.new_password_sent');
  		uni_redirect($this->authConfig['url_login']);
  	} catch (NoUserException $e) {
  		set_flash_error('auth.error.wrong_forgot_password_data');
