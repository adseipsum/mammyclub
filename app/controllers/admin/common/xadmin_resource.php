...
class xAdmin_Resource extends Base_Admin_Controller {
   ...
   * Upload resource ajax.
   */
  public function add_file_resource() {
    $this->fileoperations->set_upload_lib_config_value("allowed_types", "zip|rar|pdf|doc|xls|docx|xslx|exe");
    $this->fileoperations->set_base_dir('./web');
    $this->load->helper('common/itirra_ajax');
    // Upload Image
    try {
      if ($this->fileoperations->upload('file', TRUE)) {
        $file = new Resource();
        $file->fromArray($this->fileoperations->file_info);
        ManagerHolder::get('Resource')->insert($file);
      }
    } catch (Exception $e) {
      die('Error: ' . $e->getMessage() . '<br/> <a href="JavaScript:history.go(-1)">back</a>');
    }
    $referrer = get_referrer();
    redirect($referrer . '?file_path=' . site_file_url($file) . '&file_name=' . $file['file_name']);
  }
