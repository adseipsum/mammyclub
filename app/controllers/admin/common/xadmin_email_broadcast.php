...
class xAdmin_Email_Broadcast extends Base_Admin_Controller {
  ...
  protected function postSave(&$entity){
    ...
    // processing CSV with recipents list
    ...
    // processing recipents text list
    if(isset($_POST['recipents_list']) && !empty($_POST['recipents_list'])) {
      // matching emails by regex
      $matchCount = preg_match_all("/[_a-z0-9!=%-.]+@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,6})/i", $_POST['recipents_list'], $matches);
      if($matchCount > 0) {
        $emails = $matches[0];
    ...
    ManagerHolder::get('Broadcast')->updateById($entity['id'], 'recipents_count', count($recipents));
