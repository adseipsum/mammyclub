...
abstract class Base_Admin_Controller extends Controller {
  ...
  /** Entity add popup view. */
  protected $itemPopupView = 'add_entity_popup';

  /** Actions. */
  protected $actions = array();

  /** Additional Actions. Simple Array. Ex. array('view', 'print')*/
  protected $additionalItemActions = array();
  ...
  protected $additionalExportFilters = array();
  ...
  public function Base_Admin_Controller() {
    ...
    $this->load->helper('common/itirra_text');
  ...
  protected function init() {
    ...
    foreach ($this->additionalItemActions as $action) {
      $actions[$action] = $this->adminBaseRoute . '/' . strtolower($this->entityUrlName) . '/' . $action;
    }
    ...
    if (!$admin && isset($_COOKIE['remember_key']) && !empty($_COOKIE['remember_key'])) {
      $keyIsValid = FALSE;
      $admins = ManagerHolder::get('Admin')->getAll('e.*');
      foreach ($admins as $adm) {
        $rememberKeyValue = md5($adm['id'] . 'Itirra+salt' . 'remember_key');
        if ($rememberKeyValue == $_COOKIE['remember_key']) {
          $this->session->set_userdata(self::LOGGED_IN_ADMIN_SESSION_KEY, $adm);
          $admin = $adm;
          $keyIsValid = TRUE;
          break;
        }
    ...
    && $this->uri->segment($segment>1?$segment-1:1) != "unsubscribe") {
      if (uri_string() != '/' . $this->adminBaseRoute && empty($_POST)) {
        $this->session->set_userdata(self::SAVED_URL_SESSION_KEY, current_url() . get_get_params());
  ...
  public function index($page = "") {
    ...
    if ($this->perPage != 'all') {
      ...
      //if (count($res->data) == 1 && isset($_GET['q']) && isset($this->actions['edit'])) {
        //$firstElement = array_shift($res->data);
        //redirect($this->adminBaseRoute . '/' . strtolower($this->entityUrlName) . '/add_edit/' . $firstElement['id']);
      //}
        ...
        foreach ($_GET as $k => $v) {
          if (array_key_exists($k, $entity)) {
            ...
            if (isset($this->filters[$k])) {
              ...
              $refEnt = ManagerHolder::get($refEntityName)->getOneWhere(array($field => $v), 'e.*');
  ...
  public function add_popup() {
    ...
    $this->layout->set("fields", $this->fields);
    $this->layout->set("processLink", $this->adminBaseRoute . '/' .  strtolower($this->entityUrlName) . '/add_popup_process');
    $this->preAddEditView($entity);
    $this->layout->set("entity", $entity);
    $this->layout->view($this->itemPopupView);
  }
   ...
   * Add popup process.
   */
  public function add_popup_process() {
    $entity = $this->createEntityPOST();
    $this->loadAndResizeImages($entity);
    $this->loadFiles($entity);
    $this->loadVideos($entity);
    $entity = $this->addEditEntity($entity);
    $nameField = ManagerHolder::get($this->managerName)->getNameField();
    die( '<html><head><script type="text/javascript">
        window.opener.$("#'.$_GET['eid'].'").prepend("<option value=\"' . $entity['id'] . '\" selected=\"selected\">' . $entity[$nameField] . '</option>").trigger("liszt:updated");
        window.close();
        </script></head><body></body></html>');
  }
        ...
        switch ($field['type']) {
          ...
          case 'select2':
            if (isset($field['relation'])) {
              ...
              if($field['type'] == 'select') {
                $options = ManagerHolder::get($field['relation']['entity_name'])->getAsViewArray(array(), $nameField, $concatField, isset($field['relation']['where_array']) ? $field['relation']['where_array'] : null, '', isset($field['relation']['concat_separator']) ? $field['relation']['concat_separator'] : ',');
                if($options){
                  $field['options'] += $options;
                }
              }
              if($field['type'] == 'select2') {
                if(isset($entity[$fieldName]) && !empty($entity[$fieldName])) {
                  $field['options'] += array($entity[$fieldName]['id'] => $entity[$fieldName][$nameField]);
                }
              }
          ...
          case 'multipleselect_chosen':
            if (isset($field['relation'])) {
              ...
              $field['from'] = array();
              if (ManagerHolder::get($field['relation']['entity_name']) instanceof BaseTreeManager) {
                $options = ManagerHolder::get($field['relation']['entity_name'])->getAsViewArray($whithoutIds, $nameField, $concatField, $whereArray, isset($field['relation']['concat_separator']) ? $field['relation']['concat_separator'] : '-');
              } else {
                $options = ManagerHolder::get($field['relation']['entity_name'])->getAsViewArray($whithoutIds, $nameField, $concatField, $whereArray, '', isset($field['relation']['concat_separator']) ? $field['relation']['concat_separator'] : ',');
              }
              if($options){
          ...
          case 'checkbox_tree':
            if (isset($field['relation'])) {
              // Setting fk options
              $field['name_field'] = isset($field['relation']['name_field']) ? $field['relation']['name_field'] : 'name';
              $field['options'] = ManagerHolder::get($field['relation']['entity_name'])->getAsArray();
            }
            break;
          case 'enum':
  ...
  /**
   * Get Select2 Values Ajax
   * @param string $field
   */
  public function get_select2_field_values_ajax($field) {
    $result = array();
    if (isset($this->fields[$field]) && isset($_GET['q'])) {
      $field = $this->fields[$field];
      if (isset($field['relation'])) {
  ...
  protected function addEditEntity($entity) {
    ...
    if (empty($_POST["id"])) {
      try {
        ...
        $this->addAdminLogRecord($entity, 'add');
        set_flash_notice('admin.messages.' . strtolower($this->entityName) . '.add');
      } catch (Exception $e) {
        save_post();
        log_message('error', $e->getTraceAsString());
        if($e->getCode() == DOCTRINE_DUPLICATE_ENTRY_EXCEPTION_CODE && preg_match("/Duplicate entry '(.+?)' for key '([\S]+)'/", $e->getMessage(), $matches)) {
          $key = lang_exists('admin.add_edit.' . strtolower($this->entityName) . '.' . $matches[2]) ? (lang('admin.add_edit.' . strtolower($this->entityName) . '.' . $matches[2])) : '';
          set_flash_error(lang("admin.error.duplicate", array('value' => $matches[1], 'key' => $key)));
        } else {
      ...
      try {
        ...
        $this->addAdminLogRecord($entity, 'edit');
        set_flash_notice('admin.messages.' . strtolower($this->entityName) . '.edit');
      } catch (Exception $e) {
        save_post();
        log_message('error', $e->getTraceAsString());
        if($e->getCode() == DOCTRINE_DUPLICATE_ENTRY_EXCEPTION_CODE && preg_match("/Duplicate entry '(.+?)' for key '([\S]+)'/", $e->getMessage(), $matches)) {
          $key = lang_exists('admin.add_edit.article.' . $matches[2]) ? (lang('admin.add_edit.article.' . $matches[2]) . ' = ' ) : '';
          set_flash_error(lang("admin.error.duplicate", array('value' => $matches[1], 'key' => $key)));
        } else {
          set_flash_error($e->getMessage());
        }
  ...
  public function view($id) {
    if (isset($this->fields['page_url'])) {
      $e = ManagerHolder::get($this->managerName)->getById($id, 'id, page_url');
      redirect($e['page_url']);
    } else {
      show_error('No "page_url" field. Please overwrite the view() method.');
    }
  }
    ...
    foreach ($this->fields as $fieldName => $params) {
      if ($params["type"] == "image") {
        ...
        if ($image) {
          ...
          $rels = ManagerHolder::get($refEntityName)->getRelations();
          $thisRellAlias = array_search($this->entityName, $rels);
          $fks = ManagerHolder::get($refEntityName)->getForeignKeys();
          $fk = $fks[$thisRellAlias]['local'];
          $ref[$fk] = $entity['id'];
  ...
  protected function loadAndResizeImage($fieldName) {
    try {
      if ($this->fileoperations->upload($fieldName, FALSE)) {
        ...
        $imgArr = $this->fileoperations->file_info;

        // Check for base_image_url
        $baseImgUrl = $this->config->slash_item('image_domain');
        if(!empty($baseImgUrl)) {
          $imgArr['domain'] = $baseImgUrl;
        }

        $image = ManagerHolder::get("Image")->createEntityFromArray($imgArr);
        ...
        $this->optimizeImages($imgArr, $thumbs);
   ...
   * @param array $thumbs
   */
  private function optimizeImages($imageData, $thumbs = array()) {
    $filePath = $imageData['file_path'] . $imageData['file_name'];
    $ext = '.' . array_pop(explode('.', $filePath));
    $images = array($filePath);
    if (!empty($thumbs)) {
      foreach ($thumbs as $k => $v) {
        $images[] = str_replace($ext, $k . $ext, $filePath);
      }
    }
    foreach ($images as $i) {
      ...
      if ($params["type"] == "file") {
        try {
          ...
          if ($this->fileoperations->upload($fieldName, FALSE, './web')) {
            $file = ManagerHolder::get("Resource")->createEntityFromArray($this->fileoperations->file_info);
            $file["id"] = ManagerHolder::get("Resource")->insert($file);
            $entity[$fieldName] = $file;
            $this->postLoadFiles($entity, $file);
          }
        } catch (Exception $e) {
   ...
   * @return void
   */
  protected function postLoadFiles(&$entity, &$file) {
  }
   ...
   * @return void
   */
  public function delete_file($fileId, $fileEntityName = 'Resource') {
  ...
  protected function setFilter() {
    if (empty($this->filters) && empty($this->dateFilters)) return;
    ...
    if(!empty($this->filters)) {
      ...
      $filterValues = array();
      foreach ($this->filters as $key => $value) {
        $filterValues[$key] = ManagerHolder::get($this->managerName)->getFilterValues($key);
      }
    }
    ...
    foreach ($this->filters as $key => &$value) {
      ...
      //       if(isset($this->fields[$key]) && $this->fields[$key]['type'] == 'checkbox') {
      //         $value = (bool)$value;
      //       }
    }
  }
  ...
  private function batch_update($ids) {
    ...
    if($updateValues) {
      $updateValues = array_make_nested($updateValues);
      $count = 0;
          ...
          if($needsFetching) {
            ...
            foreach ($updateValues as $k => $v) {
              ...
              if (is_array($v)) {
                ...
                if (isset($fks[$k])) {
                  if ($fks[$k]['type'] == 1) {
                    $e[$k] = get_array_vals_by_second_key(isset($e[$k]) ? $e[$k] : array(), 'id');
                    foreach ($v as $kk => $vv) {
                      if ($vv == 'NULL') {
                        $e[$k] = array();
                      } else {
                    ...
                    foreach ($v as $kk => $vv) {
                      if ($vv == 'NULL') {
                        $e[$fks[$k]['local']] = new Doctrine_Null();
                      } else {
                        $e[$fks[$k]['local']] = $vv;
                      }
                    }
  ...
  public function next($id) {
    ...
    foreach ($params as $k => $v) {
      if (strpos($k, 'sort_') === 0) {
        unset($params[$k]);
        $sortBy = substr($k, 5, strlen($k));
        $manager->setOrderBy($sortBy . ' ' . $v);
      }
    }
          ...
          foreach ($ftrans as $field) {
            ...
            if (isset($field['index']) && isset($row[$field['index']])) {
              ...
              if ($rowProcessValue !== null) {
                ...
                if (($field['val']['type'] == 'multipleselect' || $field['val']['type'] == 'multipleselect_chosen' || $field['val']['type'] == 'select' || $field['val']['type'] == 'select2') && !empty($row[$field['index']])) {
                    ...
                    if (!isset($importFilters[$field['key']]) || empty($importFilters[$field['key']])) {
                      ...
                      if (get_class($entityObject[$field['key']]) != 'Doctrine_Collection') {
                        ...
                        foreach ($entityObject->getTable()->getRelations() as $relName => $relObject) {
                          if ($field['key'] == $relObject->getLocal() || $field['key'] == $relObject->getAlias()) {
          ...
          try {
            if (count(array_keys($entity)) > 1) {
              ...
              if (isset($entity['id']) || !empty($entity['id'])) {
                ...
                $this->addAdminLogRecord($entity, 'add', NULL, 'import');
                $addCount++;
              }
            }
          } catch (Exception $e) {
   ...
   */
  protected  function import_zip_process() {
  ...
  public function export() {
    ...
    if (!empty($this->additionalExportFilters)) {
	    $exportFilters = array_merge($this->additionalExportFilters, $exportFilters);
    }
  ...
  protected function preProcessPost() {
    if (count($_POST) > 0) {
      foreach ($this->fields as $name => $params) {
        ...
        if ($params["type"] == "multipleselect" || $params["type"] == "multipleselect_chosen" || $params["type"] == "related" || $params["type"] == "checkbox_tree") {
          if (!isset($_POST[$name])) {
            $_POST[$name] = array();
          }
        }
        ...
        // Select
        if (($params["type"] == "select" || $params["type"] == "select2") && isset($params['relation'])) {
          // If the field is named after an alias with a 1 to many relation
          $fKeys = ManagerHolder::get($this->managerName)->getForeignKeys();
          if (isset($fKeys[$name]) && $fKeys[$name]['type'] == 0 && isset($_POST[$name])) {
            if (empty($_POST[$name])) {
              $_POST[$fKeys[$name]['local']] = new Doctrine_Null();
            } else {
        ...
        // checkbox tree
        if ($params["type"] == "checkbox_tree") {
          $ids = array();
          foreach($_POST[$name] as $id => $isChecked) {
            if($isChecked) {
              $ids[] = $id;
            }
          }
        ...
        if (isset($_POST[$name])) {
          if (($params["type"] == "date"  || $params["type"] == "select" || $params["type"] == "select2" || $params["type"] == "input_integer" || $params["type"] == "input_double" || $params["type"] == "enum") && empty($_POST[$name]) && $_POST[$name] !== '0') {
            $_POST[$name] = null;
          }
        }


        if ($params["type"] == "datetime" && isset($_POST[$name])) {
          // date is not set
          if(empty($_POST[$name][0])) {
            ...
            $datetime = $_POST[$name];
            unset($_POST[$name]);
            $datetime[1] = !isset($datetime[1]) ? '00' : $datetime[1];
            $datetime[2] = !isset($datetime[2]) ? '00' : $datetime[2];

            $datetime = $datetime[0] . ' ' . $datetime[1] . ':' . $datetime[2] . ':00';;
            $_POST[$name] = $datetime;
          }
        }
        ...
        // getting field info
        if(strpos($name, '.') !== FALSE) {
          // this is related entity field
          $fieldInfo = FALSE;
          $field = explode('.', $name);
          $relationAlias = $field[0];
          $fieldName = $field[1];
          $fKeys = ManagerHolder::get($this->managerName)->getForeignKeys();
          $relation = isset($fKeys[$relationAlias]) ? $fKeys[$relationAlias] : FALSE;
          if($relation) {
            $fieldInfo = ManagerHolder::get($relation['class'])->getFieldInfo($fieldName);
          }
        } else {
          // it's a native field
          $fieldInfo = ManagerHolder::get($this->managerName)->getFieldInfo($name);
        }

        if($fieldInfo && isset($fieldInfo['unique']) && $fieldInfo['type'] == 'string' && !isset($fieldInfo['notnull'])) {
          // unique but nullable string.
  ...
  public function message_properties($lang = '') {
    ...
    foreach ($this->fields as $k => $v) {
      if ($v['type'] == 'multipleselect' || $v['type'] == 'multipleselect_chosen') {
   ...
   * @param $action
   */
  protected function addAdminLogRecord($entity, $action, $entityName = NULL, $comment = '') {
    if(class_exists('AdminLog')) {
      if(!$entityName) {
        $entityName = $this->entityName;
      }
      $admin = $this->session->userdata(self::LOGGED_IN_ADMIN_SESSION_KEY);
      $logRecord = array('entity_name' => $entityName, 'action' => $action, 'admin_id' => $admin['id'], 'description' => '');
      if($action != 'delete' && isset($entity['id'])) {
        $logRecord['entity_id'] = $entity['id'];
      }
      // description
      $nameField = ManagerHolder::get($entityName)->getNameField();
      if(isset($entity[$nameField]) && !empty($entity[$nameField])) {
        if (isset($this->fields[$nameField]['type']) && $this->fields[$nameField]['type'] == 'input') {
          $logRecord['description'] = $entity[$nameField];
        }
      }
      if($comment) {
        $logRecord['description'] .= '[' . lang($comment) . ']';
      }
      ManagerHolder::get('AdminLog')->insert($logRecord);
    }
  }

  /**
   * Detailed view of entity
   * @param $entityId
   */
  public function detailed_view($entityId) {
    $this->checkPermissions('_detailed_view');
    $this->disableAllFields();
    $entity = $this->createEntityId($entityId);
    $this->itemView = 'detailed_view_entity';
    $this->setAddEditDataAndShowView($entity);
  }
   ...
   * Disable all fields
   */
  protected function disableAllFields() {
    foreach ($this->fields as &$field) {
      if (!isset($field['attrs'])) $field['attrs'] = array();
      $field['attrs']['disabled'] = 'disabled';
      if ($field['type'] == 'tinymce') $field['type'] = 'textarea';
    }
    unset($field);
  }
