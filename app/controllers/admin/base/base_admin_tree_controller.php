...
abstract class Base_Admin_Tree_Controller extends Base_Admin_Controller {
  ...
  protected function setViewParamsIndex(&$entities, $hasSidebar) {
    ...
    $this->layout->set("current_item_id", $this->session->flashdata('item_id'));
  }
  ...
  public function add_child($parentId, $entityId = null) {
    if ($entityId) {
      redirect($this->adminBaseRoute . '/' . strtolower($this->entityName) . '/add_edit/' . $entityId . get_get_params());
    }
  ...
  public function change_root_order_process() {
    ...
    foreach ($_POST['roots'] as $rId) {
      ...
      $root = ManagerHolder::get($this->entityName)->getById($rId, 'e.*');
      $decendants = ManagerHolder::get($this->entityName)->getDescendants($root['id'], null, FALSE, Doctrine::HYDRATE_ARRAY, 'e.*');
      if ($decendants) {
  ...
  public function add_edit($entityId = null, $parentId = null) {
    ...
    if ($entityId) {
      ...
      $params = rtrim($params, ',');
      $entity = ManagerHolder::get($this->entityName)->getById($entityId, $params);
    }
    ...
    if ($parentId) {
      ...
      if (isset($this->fields[$this->urlField]) && isset($this->fields[$this->urlField]['attrs']['startwith'])) {
        $parent = ManagerHolder::get($this->entityName)->getById($parentId, 'e.*');
  ...
  public function add_edit_process() {
    ...
    if (empty($_POST["id"])) {
      // ADD
      try {
        $id = ManagerHolder::get($this->entityName)->insert($entity, FALSE);
      ...
      if (empty($_POST["parent_id"])) {
        ...
        if (array_key_exists('priority', $entityArr)) {
          ...
          $root = ManagerHolder::get($this->entityName)->getById($rootId, 'e.*');
      ...
      if (isset($_POST['save_and_return_to_list'])) {
        // For JS opening tree at saved position
        $this->session->set_flashdata('item_id', $id);
        redirect($this->adminBaseRoute . '/' .  strtolower($this->entityUrlName));
      }
      ...
      if (empty($_POST["parent_id"])) {
        ...
        if ($_POST["parent_id"] != $entity['id'] && $_POST["parent_id"] != $parentId) {
          ...
          $entityArr = $entity->toArray();
          if (array_key_exists('priority', $entityArr)) {
            $newParent = ManagerHolder::get($this->entityName)->getById($_POST["parent_id"], 'e.*');
      ...
      if (isset($_POST['save_and_return_to_list'])) {
        // For JS opening tree at saved position
        $this->session->set_flashdata('item_id', $entity['id']);
        redirect($this->adminBaseRoute . '/' .  strtolower($this->entityUrlName));
      }
  ...
  public function add_popup_process() {
    ...
    die( '<html><head><script type="text/javascript">
        window.opener.$("#'.$_GET['eid'].'").prepend("<option value=\"' . $entity['id'] . '\" selected=\"selected\">' . $entity['name'] . '</option>").trigger("liszt:updated");
        window.close();
