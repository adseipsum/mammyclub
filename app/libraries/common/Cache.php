...
class Cache {
   ...
   */
  function set_cache_dir($dirName) {
    $this->options['dir'] = $dirName;
  }
  ...
  function save($cache_id, $data, $cache_group = NULL, $ttl = NULL) {
    ...
    if ($f1) {
      ...
      if (function_exists('igbinary_serialize')) {
        fwrite($f2, igbinary_serialize($data));
      } else {
      ...
      log_message('error', 'Cache library: error opening ' . $expiry_file . ' for write access!!!!!!');
    }
  }
  ...
  function get($cache_id, $cache_group = NULL, $skip_checking = FALSE) {
    ...
    if (function_exists('igbinary_unserialize')) {
      return igbinary_unserialize(file_get_contents($cache_file));
    } else {
  ...
  function remove_group($cache_group) {
    ...
    //	Empty the directory
    //
    if(!is_dir($group_dir) || !$dh = @opendir($group_dir)) {
      return;
    }
