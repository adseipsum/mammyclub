...
class Auth2 {
   ...
   * @param string $config
   */
  public function Auth2($options = array()) {
    $this->ci =& get_instance();
    $configName = 'auth';
    if (isset($options['config'])) {
      $configName = $options['config'];
    }
    $this->ci->load->config($configName);
   ...
   * @param array $data
   */
  public function login($data = null, $dataIsVerified = FALSE, $checkConfirmation = TRUE, $isAdmin = FALSE) {
    ...
    foreach ($loginFields as $key => $value) {
      if (isset($data[$key])) {
        if (!$dataIsVerified) {
          if ($key == 'password') {
            $data['password'] = $this->preparePassword($data['password']);
          }
          if ($key == 'phone') {
            $data['phone'] = preprocess_phone($data['phone']);
          }
    ...
    if ($entity['is_banned']) {
      $this->increaseAuthAttempts('login');
      throw new UserBannedException($entity['banned_reason']);
    }
    ...
    if ($checkConfirmation && $this->config['email_confirmation'] && !$entity['email_confirmed'] && !$this->config['do_login_after_register']) {
      $this->sendEmailConfirmation($entity);
      throw new NotConfirmedException(NotConfirmedException::NOT_CONFIRMED_EMAIL);
    }

    // Check if phone confirmed
    if ($checkConfirmation && $this->config['phone_confirmation'] && !$entity['phone_confirmed'] && !$this->config['do_login_after_register']) {
  ...
  /**
   * Did user confirmed privacy policy.
   * @return bool
   */
  public function isConfirmedPolicy() {
    return $this->getAuthEntity()['is_privacy_policy_confirmed'] ? TRUE : FALSE;
  }
   ...
   * @throws NoUserException
   */
  public function forgotPassword($data, $sendView = FALSE, $field = 'email') {
    // Check if email confirmed
    if (!$this->config['email_confirmation'] && $this->config['phone_confirmation']) {
      $field = 'phone';
    }
    if (!isset($data[$field])) {
      throw new Exception('No data in field: ' . $field);
    }

    if ($field == 'email') {
      $this->checkEmail($data[$field]);
      $where = array('email' => $data[$field]);
    }

    if ($field == 'phone') {
      $data[$field] = preprocess_phone($data[$field]);
      $where = array('phone' => $data[$field]);
    }
  	...
  	$entity['password'] = $this->preparePassword($newPassword);
  	$this->updateEntity($entity);
  	...
  	if ($field == 'phone') {
      $this->sendSMSForgotPassword($entity, $newPassword);
  	}
  }
  ...
  private function sendEmailForgotPassword($entity, $newPassword, $sendView) {
  	...
  	if($sendView == FALSE) {
  	  ...
  	  ManagerHolder::get('Email')->sendView($entity['auth_info']['email'], 'forgot_password', $data);
  	}
  }
   ...
   * @param bool $sendView
   */
  private function sendSMSForgotPassword($entity, $newPassword) {
  	$data = array(
      'password' => $newPassword
  	);
	  // sending sms
    $url = $this->config['sms_url'];
    $data = $this->config['sms_data'];

    $data['TO'] = $entity['phone'];
    $data['MESSAGE'] = rawurlencode(lang('auth.sms.new_password', array('PASSWORD' => $newPassword)));


    $url = kprintf($url, $data, -1, FALSE);
    $ch = curl_init($url);
     ...
     if ($rememberMeCookie != "") {
       ...
       if (isset($rememberMeCookie['key']) && isset($rememberMeCookie['id'])) {
         if ($this->checkRememberMeKey($rememberMeCookie['key'], $rememberMeCookie['id'])) {
           $entity = $this->manager->getById($rememberMeCookie['id'], $this->what);
           if ($entity) {
             $this->login($entity, TRUE);
           }
         }
       }
     }
