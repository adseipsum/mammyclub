...
class DoctrineLoader {
  ...
  public function __construct() {
    ...
    if (isset($db['default']['doctrine_cache']) && $db['default']['doctrine_cache']) {
      $host = isset($db['default']['doctrine_memcache_host']) ? $db['default']['doctrine_memcache_host'] : 'localhost';
      $port = isset($db['default']['doctrine_memcache_port']) ? $db['default']['doctrine_memcache_port'] : 11211;
      $this->enableMemcache($host, $port);
    }
   ...
   * Enable Memcache.
   */
  private function enableMemcache($host = 'localhost', $port = 11211) {
    $servers = array(
      'host' => $host,
      'port' => $port,
      'persistent' => true
