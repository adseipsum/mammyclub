...
class Events {
  ...
  protected static $debug = FALSE;
	...
	public static function trigger($event, $data = '', $return_type = 'string') {
      ...
      $className = $event[0] . 'Events';
      if (file_exists(APPPATH  . "logic/events/" . $className . ".php")) {
        require_once APPPATH . "logic/events/" . $className . ".php";
      }

      $call = array($className, $event[1]);
      if (is_callable($call)) {
        $result = call_user_func($call, $data);

//        $isLogging = call_user_func(array($className, 'isLogging'));
        $isLogging = TRUE;
        ...
        if ($isLogging) {
          ...
          $eventLog = array();
          $eventLog['event_model'] = $event[0];
          $eventLog['event_method'] = $event[1];
          $eventLog['created_at'] = date(DOCTRINE_DATE_FORMAT);
          if (!empty($admin)) {
            $eventLog['change_by'] = 'admin';
            $eventLog['admin_id'] = $admin['id'];
          }

          if (is_array($data)) {
            if (isset($data['id'])) {
              $eventLog['entity_id'] = $data['id'];
            }
            $data = json_encode($data);
          }
          $eventLog['data'] = $data;
          if (is_bool($result)) {
            $eventLog['is_success'] = $result;
          }
          if (is_array($result)) {
            $eventLog['is_success'] = $result['is_success'];
            $eventLog['result'] = $result['result'];
          }
          if (isset($result['search_field_value'])) {
            $eventLog['search_field_value'] = $result['search_field_value'];
          }

          $eventLog['id'] = ManagerHolder::get('EventLog')->insert($eventLog);
          return $eventLog;
        }
      }
    }
