...
class LiqPaySDK
{
    ...
    {
        $public_key = $options[0];
        $private_key = $options[1];
        if (empty($public_key)) {
     ...
     * @throws InvalidArgumentException
     */
    public function cnb_form($params, $showButton = TRUE)
    {
         $language = 'ru';
        ...
        $button = '';
        if ($showButton) {
          $button = '<input type="image" src="//static.liqpay.ua/buttons/p1%s.radius.png" name="btn_text" />';
        }
    ...
    public function transaction_is_valid(){
      $generatedSignature = $this->str_to_sign($this->_private_key . $_POST['data'] . $this->_private_key);
