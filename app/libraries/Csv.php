...
class Csv {
  ...
  protected $maxLineLength = 512000;
  ...
  public function addHeader(array $values) {
    if (!empty($values)) {
      ...
      $this->addRowToCsvData($values, TRUE);
    }
  }
   ...
   * @param string $fileName
   */
  public function saveFile($filePath = './web/example.csv', $changeEncoding = TRUE) {
    $result = $this->data;
  ...
  public function readRow() {
    ...
    if ($data !== FALSE) {
      if ($this->inputFileEncoding != "utf-8") {
        foreach ($data as &$el) {
          $el = @iconv("cp1251", "utf-8//IGNORE", $el);
        }
      }
    }
  ...
  /**
   * Get row count without header row
   */
  public function getRowCount($filePath) {
   ...
   * @param array $values
   */
  protected function addRowToCsvData(array $values, $encodeAll = FALSE) {
    $outstream = fopen("php://temp", 'r+');
    if ($encodeAll == TRUE) {
      foreach ($values as &$v) {
        $v = "\"$v\"";
      }
      fputcsv($outstream, $values, $this->sep, chr(0));
    } else {
...
//      $result = preg_replace("/\r?\n/", "\\n", $result);
   ...
   */
  public function setInputFileEncoding($encoding) {
    if(!empty($encoding)) {
      $this->inputFileEncoding = $encoding;
    }
  }
