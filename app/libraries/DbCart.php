...
//     }
   ...
   * @return array
   */
  public function get_cart_item($itemId) {
    $result = array();
    foreach ($this->cart['items'] as $i => $item) {
      if ($item['id'] == $itemId) {
  ...
  private function _update_cart() {
    ...
    $userId = $this->_get_cart_owner();
    if(!empty($userId)) {
      $this->cart['user_id'] = $userId;
      unset($this->cart['user']); // This is just for cases if user_id changed
    }
  ...
  private function _create_cart() {
    ...
    $this->cart['user_id'] = $this->_get_cart_owner();
   ...
   * Get cart owner
   */
  private function _get_cart_owner() {
    $this->CI->load->library('Auth');
    return $this->CI->auth->getAuthEntityId();
  }
