...
class Sitemap {
  ...
  public function generate() {
    ...
    $this->saveSitemapIndex($xml);

    // Generate separate sitemap_product.xml
    $this->sitemapName = 'sitemap_shop';
    $xml = self::SITEMAP_INDEX_XML_HEADER;
    $urls = $this->generateForEntity('Product');
    foreach ($urls as $u) {
      $xml .= '<sitemap>';
      $xml .= '<loc>' . $u . '</loc>';
      $xml .= '</sitemap>';
  ...
  public function generateForEntity($entityName, $defaults = NULL) {
    ...
    foreach($entities as $e) {
      $xmlUrl  = '<url>';
      $loc = htmlspecialchars(site_url($e['loc']));
      if($entityName == 'Product') {
        $loc = htmlspecialchars(shop_url($e['loc']));
      }
      $xmlUrl .= '<loc>' . $loc . '</loc>';
